...
router.get("/read", (req, res) => {
    ...
    const bws = req.headers["bw"] || req.query.bw;
    ...
    if (bw) {
        ...
        for (let i = 0; i < chunk; i++) {
            pixel = frames[currentFrame][y][x];
            const bit = (pixel[0] === '1' || pixel[2] === '1' || pixel[4] === '1') ? '1' : '0'; // simple threshold on first bit for BW
...
router.post("/reset", (req, res) => {
    console.log("reset values to 0");

    currentFrame = 0;
    x = 0;
    y = 0;

    res.json({ value: "00000000", status: "reset" });
});
